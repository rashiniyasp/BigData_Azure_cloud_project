...
*For detailed architecture, see [Architecture Diagram](Docs/Architecture_Pipeline.png)*
...
│   ├── PLots2.pbix
│   └── Screenshot*.png           # 4 dashboard views
│
├── notebooks/                     # Databricks transformation code
│   ├── DataIngestionToDB.ipynb
│   ├── Databricks_Tranformation_2.ipynb
│   └── JSONforeachinput (1).json
│
├── data/                          # Sample datasets (links to full data)
...
- Supports HTTP (GitHub), MySQL, MongoDB with unified Copy Activity pattern

**Key File:** `notebooks/JSONforeachinput (1).json`
...
**Notebooks:**
- `DataIngestionToDB.ipynb` - MySQL data loading
- `Databricks_Tranformation_2.ipynb` - Bronze → Silver transformations
...
- Power BI connects via Direct Query
